...
%union {
    float                     yfloat;
    char                      *ychar;
    SdfDelayType      ydelayType;
    SdfValue          yvalue;
    SdfDelayValue     ydelayValue;
    SdfDelayValueList ydelayValueList;
    SdfDelayDef       *ydelayDef;
    SdfIoPath         yioPath;
    SdfPortSpec       yportSpec;
}
...
void sdf_filefreeSdfDelayDef(SdfDelayDef *defs) {
    ...
    while (def) {
        SdfDelayDef *prev = def;
        def = def->next;
        delete [] prev->pspec.port;
        delete [] prev->p1;
        delete [] prev->p2;
        delete prev;
    }
